...
2 CAIXA 
	Esta será uma opção bastante utilizada, você indicará o número da mesa que deseja visualizar e o programa listará os pedidos desta mesa e o valor da conta.
...
Após isso, você poderá escolher a opção da ação que deseja realizar, no seu menu há as seguintes opções:
1 ABRIR PEDIDO
	Nesta opção você terá acesso a uma fila de pedidos que há para fazer, após a realização de determinado pedido você deve ir para opção 2 que informa ao programa que aquele pedido está concluído.
2 PEDIDO CONCLUIDO
	Esta opção é utilizada para informar ao programa que determinado pedido está pronto, nela você também indica se deseja ir para o próximo pedido.
